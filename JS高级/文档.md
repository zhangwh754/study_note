...
- 显示绑定1：apply、call，`本次`函数的执行，this会变成绑定的第一个参数（不能是null或者undefined，不然会指向window）
...
- 显示绑定2：bind，`返回的新函数`，this会变成绑定的第一个参数（不能是null或者undefined，不然会指向window），返回的新函数this指向改变一直有效，且无法再改变
...
- 对象的隐式原型（\_\_proto__）=对象的构造函数的显式原型（prototype）
...
- Student对象，一些属性在原型链上，打印看不到（name属性）
- 父类的（People）引用属性是公用的，进行修改会影响别的子类
- 传参很难
...
- 静态方法只能类名.方法名调用；实例方法只能实例化后，通过对象名.方法名调用。
...
一种特殊的函数调用方式，第一个参数是字符串被${}隔开的每一项组成的数组
...
[10分钟讲明白WeakSet和WeakMap的弱引用 - 掘金 (juejin.cn)](https://juejin.cn/post/7041574685363945479)
...
obj.age++
/* 模拟在这里监听到了obj.age的改变，执行绑定的回调函数 */
const targetMap = m1.get(obj)
const fns = targetMap.get('age').forEach(fn => fn())
```

<img src="img/image-20221003182803300.png" alt="image-20221003182803300" style="zoom: 50%;" />
...
// 相当于
// const words = strArr.map(word => word.split(' ')).flat(2)
...
类似||，但和||的不同是只有undefined和null会进入默认值，而0和空字符串会视作true
...
能监听获取、修改，但是不能监听删除、新增操作（Vue2响应式的缺点）
...
- 传入的this如果是null或undefined，会变成window
- 优化方案可以把赋值属性改成defineProperty赋值，不可枚举
...
Function.prototype.myApply = function (thisArg, args = []) {
  ...
  const s = Symbol()
  Object.defineProperty(thisArg, s, {
    enumerable: false,
    value: fn
  })
...
const add = function (num1, num2) {
  ...
  return `传参${num1}+${num2}的结果是${num1 + num2}`
}

const obj = { age: 10 }
console.log('apply:', add.myApply(obj, [1, 2]))
...
console.log('call:', add.myCall(obj, 1, 3))
console.log('call:', add.call(obj, 1, 3))
/**
 *  this.age === 10
 *  call: 传参1+3的结果是4
 */
...
Function.prototype.myBind = function (thisArg, ...args) {
  ...
  function foo(...args2) {
    thisArg.fo = fo
    const res = thisArg.fo(...args, ...args2)
    delete thisArg.fo
    return res
  }
  return foo
...
const obj = { age: 11 }
const bind1 = add.myBind(obj, 1)
const bind2 = add.bind(obj, 1)
console.log('bind:', bind1(2))
console.log('bind:', bind2(2))
/**
 *  this.age === 10
 *  bind: 传参1+2的结果是3
 *  this.age === 10
